...
public class KyoShopGui extends SimpleGui {
  ...
  private String getCategoryFriendlyName(String catId) {
    switch (catId) {
      case "blocks":
        return "Khối Xây Dựng";
      case "food":
        return "Nông Sản & Thực Phẩm";
      case "materials":
        return "Nguyên Liệu Chế Tạo";
      case "equipment":
        return "Trang Bị & Vũ Khí";
      case "mob_drops":
        return "Chiến Lợi Phẩm Quái Vật";
      case "special":
        return "Đồ Đặc Biệt & Hiếm";
      default:
  ...
  private void handleBuy(ServerPlayer buyer, Item itemType, long priceInHao, int amount) {
    ...
    if (currentBal >= priceInHao) {
      ...
      if (!buyer.getInventory().add(purchasedItem)) {
        ItemEntity dropItem = new ItemEntity(buyer.level(), buyer.getX(), buyer.getY(), buyer.getZ(), purchasedItem);
        buyer.level().addFreshEntity(dropItem);
      }
