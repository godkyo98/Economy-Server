...
public class KyoEconomyCommands {
  ...
  private static void registerBal(CommandDispatcher<CommandSourceStack> dispatcher) {
    for (String alias : KyoEconomyConfig.CMD_BAL) {
      dispatcher.register(Commands.literal(alias)
          ...
          .then(Commands.argument("target", EntityArgument.player())
              .executes(context -> {
                ...
                // FIX TÊN ARGUMENT: Đã đổi "player" thành "target" để tránh Crash Server lúc chạy lệnh
  ...
  private static void registerPay(CommandDispatcher<CommandSourceStack> dispatcher) {
    for (String alias : KyoEconomyConfig.CMD_PAY) {
      dispatcher.register(Commands.literal(alias)
  ...
  private static void registerAdmin(CommandDispatcher<CommandSourceStack> dispatcher) {
    for (String alias : KyoEconomyConfig.CMD_ECO) {
      dispatcher.register(Commands.literal(alias)
          // FIX CHECK OP: Sửa lỗi không tìm thấy biến `p` và đồng bộ hóa chuẩn bảo mật 26.2
          .requires(source -> {
            try {
              if (source.getEntity() instanceof ServerPlayer player) {
                return source.getServer().getPlayerList().isOp(player.nameAndId());
              }
              return true; // Cho phép Console có quyền chạy lệnh
            } catch (Exception e) {
              return false;
            }
          })
  ...
  } // ĐÂY CHÍNH LÀ DẤU NGOẶC ĐÃ BỊ THIẾU GÂY LỖI ILLEGAL START OF EXPRESSION!

  // 4. Lệnh Shop (Mở Admin Shop hoặc Admin thêm đồ)
  private static void registerShop(CommandDispatcher<CommandSourceStack> dispatcher) {
    for (String alias : KyoEconomyConfig.CMD_SHOP) {
      dispatcher.register(Commands.literal(alias)

          // Chức năng 1: Người chơi mở Shop bình thường
          .executes(context -> {
            ServerPlayer player = context.getSource().getPlayerOrException();
            KyoShopGui gui = new KyoShopGui(player);
            gui.open();
            return 1;
          })
  ...
  // 5. LỆNH MỞ CHỢ ĐEN NGƯỜI CHƠI (/ah) - HỖ TRỢ ĐĂNG BÁN THẬP PHÂN XU THÂN THIỆN
  private static void registerAh(CommandDispatcher<CommandSourceStack> dispatcher) {
    for (String alias : KyoEconomyConfig.CMD_AH) {
      dispatcher.register(Commands.literal(alias)
          .executes(context -> {
            ServerPlayer player = context.getSource().getPlayerOrException();
